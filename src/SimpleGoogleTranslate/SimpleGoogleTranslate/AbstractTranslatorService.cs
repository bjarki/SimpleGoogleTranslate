...
namespace SimpleGoogleTranslate
{
    ...
    public abstract class AbstractTranslatorService
    {
        ...
        /// </summary>
        protected string EncodeSequences(string text, out Dictionary<string, string> encodeInfo)
        {
            encodeInfo = null;
            if (text == null) return null;

            encodeInfo = new Dictionary<string, string>();
        ...
        {
            return string.Format("{0}", random.Next(500, 1000));
        }

        /// <summary>
        /// Replaces number in text with given replacements
        /// </summary>
        protected string DecodeSequences(string text, Dictionary<string, string> encodeInfo)
        {
            if (text == null) return null;
            ...
            if (encodeInfo != null)
            {
                ...
                {
                    b.Replace(pair.Key.ToString(CultureInfo.InvariantCulture), pair.Value);
                }
            }
            return b.ToString();
        }
